...
import type { TrainingSheetExercise, TrainingSheetMessage, TrainingSheetResponse } from '@/types/interfaces/trainingSheet';

export const getTrainingSheet = async (): Promise<TrainingSheetResponse | null> => {
  try {
...
export const createTrainingSheet = async (
  ...
  try {
    ...
    Alert.alert('Erro', error.message || 'Não foi possível criar a ficha de treino');
    return false;
  }
};
