...
interface TrainingSheetWithCompletion extends Omit<TrainingSheet, 'training_sheet_exercises'> {
...
export function DayItem({ sheet, expandedDay, toggleDay, removeDay, newExercise, setNewExercise, addExercise, toggleExerciseCompletion, removeExercise }: DayItemProps) {
  return (
      ...
      {expandedDay === sheet.title && (
        <View style={styles.exerciseList}>
          ...
          <FlatList
            ...
            renderItem={({ item, index }) => <ExerciseItem item={item} index={index} day={sheet.title} toggleExerciseCompletion={toggleExerciseCompletion} removeExercise={removeExercise} />}
            scrollEnabled={false}
          />

          <AddExercise day={sheet.title} newExercise={newExercise} setNewExercise={setNewExercise} addExercise={addExercise} />
        </View>
      )}
    </View>
  );
