...
export function useTrainingSheet() {
  ...
  const [trainingSheets, setTrainingSheets] = useState<TrainingSheetWithCompletion[]>([]);
  ...
  const fetchTrainingSheet = async () => {
    setLoading(true);
    try {
      const storedSheets = await AsyncStorage.getItem(TRAINING_SHEET_STORAGE_KEY);
      if (storedSheets) {
        setTrainingSheets(JSON.parse(storedSheets));
      }
    } catch (error) {
      console.error('Error fetching training sheet from storage:', error);
      Alert.alert('Erro', 'Não foi possível carregar a ficha de treino do dispositivo');
    }
    setLoading(false);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      await AsyncStorage.setItem(TRAINING_SHEET_STORAGE_KEY, JSON.stringify(trainingSheets));
      Alert.alert('Sucesso', 'Ficha de treino salva com sucesso!');
    } catch (error) {
      console.error('Error saving training sheet to storage:', error);
      Alert.alert('Erro', 'Não foi possível salvar a ficha de treino no dispositivo');
    }
  };
